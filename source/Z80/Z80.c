...
extern byte *RAM[];
//#ifdef TURBO_R
//INLINE byte OpZ80(word A) { return OPRAM(A); }
//#else
...
#ifdef TURBO_R
#define M_SLL(Rg)      \
  R->AF.B.l=Rg>>7;Rg=(Rg<<1)|((R->User & 0x01) ? 0 : 1);R->AF.B.l|=PZSTable[Rg]
#else
#define M_SLL(Rg)      \
...
#ifdef TURBO_R
//Taken from RuMSX
//Add modify from blueMSX

#define M_MULUW(Rg1,Rg2)          \
  {                                 \
  if ((R->User)&0x01)                    \
    {                               \
    unsigned int ulResult;        \
    ulResult=(unsigned int)(Rg1.W)*Rg2.W; \
    R->DE.W=(unsigned short)(ulResult>>16);         \
    R->HL.W=(unsigned short)(ulResult&0xFFFF);               \
    R->AF.B.l &=(N_FLAG | H_FLAG);                     \
...
#define M_MULU(r1,r2)               \
  ...
  if ((R->User)&0x01)                   \
    {                               \
    R->HL.W =((unsigned short)(r1))*(r2);       \
    R->AF.B.l &=(N_FLAG | H_FLAG);    \
    if (R->HL.W) R->AF.B.l |=Z_FLAG; else R->AF.B.l &=~Z_FLAG; \
    if (R->HL.W & 0x8000) R->AF.B.l |=C_FLAG; else R->AF.B.l &=~C_FLAG; \
    }                               \
  }
#endif // TURBO_R
...
static void CodesCB(register Z80 *R)
{
  ...
  I=OpZ80(R->PC.W++);
#ifdef TURBO_R
  R->ICount -= (R->User & 0x01) ? CyclesCB_R800[I] : CyclesCB[I];
#else
  R->ICount-=CyclesCB[I];
#endif // TURBO_R
...
#define XX IX    
  ...
  I=OpZ80(R->PC.W++);
#ifdef TURBO_R
  R->ICount -= (R->User & 0x01) ? CyclesXXCB_R800[I] : CyclesXXCB[I];
#else
...
static void CodesED(register Z80 *R)
{
  ...
  I=OpZ80(R->PC.W++);
#ifdef TURBO_R
  R->ICount -= (R->User & 0x01) ? CyclesED_R800[I] : CyclesED[I];
#else
  R->ICount-=CyclesED[I];
#endif // TURBO_R
...
#define XX IX
  ...
  I=OpZ80(R->PC.W++);
#ifdef TURBO_R
  R->ICount -= (R->User & 0x01) ? CycleXX_R800[I] : CyclesXX[I];
#else
  R->ICount-=CyclesXX[I];
#endif // TURBO_R
